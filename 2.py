...
USERLIST_FILE = "userlist.json"         # Stores verified users (email + encrypted app password)
...
srv = {
    ...
    'sender_email': None,
    'app_password': None,   # Per-session override for Gmail App Password
}
...
# ═══════════════════════════════════════════════════════════════
#  FERNET ENCRYPTION  (reversible — used to protect stored app passwords)
# ═══════════════════════════════════════════════════════════════
def _get_fernet():
    """Derive a Fernet key from the server-side PASSWORD constant."""
    key_bytes  = hashlib.sha256(PASSWORD).digest()          # 32 bytes
    fernet_key = base64.urlsafe_b64encode(key_bytes)        # Fernet expects base64url
    return Fernet(fernet_key)

def encrypt_app_password(plain_pw: str) -> str:
    return _get_fernet().encrypt(plain_pw.encode()).decode()

def decrypt_app_password(enc_pw: str) -> str:
    return _get_fernet().decrypt(enc_pw.encode()).decode()


# ═══════════════════════════════════════════════════════════════
#  USERLIST  (userlist.json — email + encrypted app password)
# ═══════════════════════════════════════════════════════════════
def load_userlist():
...
def _save_userlist(users):
    with open(USERLIST_FILE, 'w') as f:
        json.dump(users, f, indent=2)

def is_email_registered(email: str) -> bool:
    return any(u['email'].lower() == email.strip().lower() for u in load_userlist())

def get_user_app_password(email: str):
    """Return decrypted app password for a registered email, or None."""
    for u in load_userlist():
        if u['email'].lower() == email.strip().lower():
            try:
                return decrypt_app_password(u['app_password_enc'])
            except Exception:
                return None
    return None

def save_verified_user(email: str, plain_app_password: str):
    """Upsert email + encrypted app password into userlist.json."""
    users     = load_userlist()
    email_key = email.strip().lower()
    enc_pw    = encrypt_app_password(plain_app_password)
    for u in users:
        if u['email'].lower() == email_key:
            u['app_password_enc'] = enc_pw
            _save_userlist(users)
            return
    users.append({'email': email.strip(), 'app_password_enc': enc_pw})
    _save_userlist(users)
...
def send_otp_email(sender_email, app_password):
    """Send a 6-digit OTP to sender_email using the provided app_password."""
    if not app_password:
        print("   OTP send error: no app password provided")
        return None, None
    otp = generate_otp()
    body = (
...
def send_final_email(sender_email, receiver_emails, file_links, message=None):
    ...
    for receiver in receiver_emails:
        ...
        try:
            with smtplib.SMTP_SSL("smtp.gmail.com", 465) as server:
                server.login(sender_email, srv.get('app_password') or '')
...
def api_otp_send():
    ...
    custom_pw = data.get('app_password', '').strip() or None
    if not email:
        return jsonify({'success': False, 'error': 'Email required'}), 400

    srv['sender_email'] = email

    # Decide which app password to use
    if custom_pw:
        # New user supplying their app password for the first time
        app_pw = custom_pw
        srv['app_password'] = custom_pw
    elif is_email_registered(email):
        # Returning user — decrypt stored password
        app_pw = get_user_app_password(email)
        if not app_pw:
            return jsonify({'success': False,
                            'error': 'Stored app password could not be decrypted — re-register'}), 500
        srv['app_password'] = app_pw
    else:
        return jsonify({'success': False,
                        'error': 'Email not registered — please provide your Gmail App Password'}), 400

    otp, sent_at = send_otp_email(email, app_pw)
    if otp:
        srv['otp']          = otp
        srv['otp_sent_at']  = sent_at
        srv['otp_attempts'] = 3
        return jsonify({'success': True})
    return jsonify({'success': False, 'error': 'Failed to send OTP — check your App Password'}), 500
...
def api_otp_verify():
    ...
    if entered == srv['otp']:
        ...
        # Persist email + encrypted app password into userlist.json
        email_to_save = srv.get('sender_email', '')
        app_pw_to_save = srv.get('app_password', '')
        if email_to_save and app_pw_to_save:
            save_verified_user(email_to_save, app_pw_to_save)
        return jsonify({'success': True})
    srv['otp_attempts'] -= 1
...
# ═══════════════════════════════════════════════════════════════
#  FLASK ROUTES — EMAIL REGISTRATION CHECK
# ═══════════════════════════════════════════════════════════════
@app.route('/api/email/check', methods=['POST'])
def api_email_check():
    data  = request.get_json()
    email = data.get('email', '').strip()
    return jsonify({'found': is_email_registered(email)})
